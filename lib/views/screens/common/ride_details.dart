...
class _RideDetailScreenState extends State<RideDetailScreen> {
  ...
  void _setupChatMessageListener() {
    ...
    _webSocketService.onChatMessageReceived = (chatMessage) {
      ...
      if ((chatMessage.senderEmail == driverEmail ||
              chatMessage.receiverEmail == driverEmail) &&
          mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Tin nhắn mới từ ${rideData.driverName}: ${chatMessage.content}',
            ),
  ...
  Future<void> _checkExistingBooking() async {
    ...
    try {
      ...
      if (activeBookings.isNotEmpty) {
        ...
        print(
          '✅ Đã tìm thấy booking đang hoạt động cho chuyến đi #$rideId: ${booking.id} - trạng thái: ${booking.status}',
        );
        ...
        if (lastCreatedBooking != null &&
          ...
          print(
            '✅ Tìm thấy booking local cho chuyến đi #$rideId: ${lastCreatedBooking.id} - trạng thái: ${lastCreatedBooking.status}',
          );
          ...
          print(
            'ℹ️ Không có booking đang hoạt động nào cho chuyến đi #$rideId',
          );
          if (!mounted) return;
          setState(() {
  ...
  void _setupBookingStatusListener(int bookingId) {
          ...
          if (snapshot.exists) {
            ...
            try {
              ...
              if (_booking?.status != firebaseBooking.status) {
                print(
                  'ℹ️ Cập nhật trạng thái local từ Firebase: ${_booking?.status} -> ${firebaseBooking.status}',
                );
                setState(() {
            ...
            print(
              'ℹ️ Booking #$bookingId chưa tồn tại trên Firebase, đang tạo mới...',
            );
    ...
    _bookingStatusSubscription = bookingRef.onValue.listen(
      (event) {
        if (event.snapshot.value != null) {
          try {
            ...
            print(
              '📡 Nhận được cập nhật từ Firebase cho booking #$bookingId - trạng thái: ${updatedBooking.status}',
            );
  ...
  Future<void> _bookRide() async {
    ...
    try {
      print(
        '🛒 Bắt đầu đặt chỗ cho chuyến đi #${rideData.id} với ${_selectedSeats} ghế',
      );
      ...
      if (booking != null) {
        print(
          '✅ Đặt chỗ thành công cho chuyến đi #${rideData.id}, booking ID: ${booking.id}',
        );
        ...
        try {
          ...
          if (driverEmail.isNotEmpty) {
            await _notificationService.sendNotification(
              'Yêu cầu đặt chỗ mới',
              'Hành khách ${booking.passengerName} đã đặt ${booking.seatsBooked} ghế cho chuyến đi của bạn.',
  ...
  Widget _buildBookingStatusContainer(Booking booking) {
    ...
    final bool isDriverConfirmed =
        rideData.status.toUpperCase() == 'DRIVER_CONFIRMED';
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed:
                            _isLoading
                                ? null
                                : () => _confirmPassengerDeparture(booking),
                        icon:
                            _isLoading
                                ? const SizedBox(
                                  width: 20,
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed:
                            _isLoading
                                ? null
                                : () => _confirmRideCompletion(booking),
                        icon:
                            _isLoading
                                ? const SizedBox(
                                  width: 20,
        ...
        await showDialog(
          ...
          builder:
              (context) => AlertDialog(
                ...
                content: const Text('Bạn có chắc chắn muốn hủy đặt chỗ không?'),
                actions: [
                  TextButton(
    ...
    try {
      ...
      if (success) {
        ...
        if (mounted) {
          ...
          setState(() {
            _booking =
                null; // Xóa booking hoàn toàn thay vì chỉ cập nhật trạng thái
        ...
        await showDialog(
          ...
          builder:
              (context) => AlertDialog(
                ...
                content: const Text(
                  'Bạn xác nhận đã sẵn sàng tham gia chuyến đi này?',
                ),
                actions: [
                  TextButton(
                    onPressed: () => Navigator.pop(context, false),
              ...
              (context) => AlertDialog(
                ...
                actions: [
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                          isDriverConfirmed ? Colors.teal : Colors.green,
                    ),
                    child: const Text('Xác nhận'),
                  ),
                ],
  ...
  Future<void> _refreshBookingStatus() async {
    ...
    try {
      ...
      if (_isBooked && _booking != null) {
        ...
        final latestBooking = await _bookingService.getBookingDetail(
          _booking!.id,
        );
        ...
        final currentBooking =
            bookings.where((b) => b.rideId == rideData.id).firstOrNull;
  ...
  Future<void> _createChatRoomWithDriver(String driverEmail) async {
    try {
      ...
      if (roomId != null && roomId.isNotEmpty) {
        ...
        if (_booking != null) {
          final bookingInfo =
              'Xin chào! Tôi đã đặt ${_booking!.seatsBooked} ghế cho chuyến đi của bạn!';
