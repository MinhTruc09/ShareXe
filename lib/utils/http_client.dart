...
class ApiClient {
  ...
  Future<Map<String, String>> _getHeaders({bool requireAuth = true}) async {
    ...
    if (token != null) {
      ...
      print('🔐 Adding auth token: Bearer ${token.length > 20 ? token.substring(0, 20) + '...' : token}');
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      try {
        ...
        if (response.headers['content-type']?.contains('text/html') == true || 
          ...
          print('📄 Raw response start: ${response.body.substring(0, min(100, response.body.length))}...');
          return;
        }
  ...
  Future<void> handleAuthError(BuildContext context) async {
    ...
    Navigator.of(context).pushNamedAndRemoveUntil('/login_passenger', (route) => false);
  }
  ...
  Future<http.StreamedResponse> multipartRequest(
    ...
    print('🌐 API Multipart $method Request: $_baseUrl$endpoint');
    print('🔑 Headers: ${headers.toString()}');
    return request.send();
  }
} 